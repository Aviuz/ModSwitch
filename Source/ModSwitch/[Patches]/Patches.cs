...
namespace DoctorVanGogh.ModSwitch {
    ...
    class Patches {
        ...
        public class ModsConfig_Reset {
            public static void Prefix() {
                try {
                    var ms = LoadedModManager.GetMod<ModSwitch>();
                    string name = LanguageKeys.keyed.ModSwitch_Reset.Translate(DateTime.Now);
                    var set = ms.AddNewSet(name);
                    set.AutoGenerated = true;
                    ms.WriteSettings();
                    Util.Log($"Auto-created ModSet {name}.");
                }
                catch (Exception ex) {
                    Util.Error($"Exception while trying to create ModSet error snapshot: {ex}");
                }
            }
        }
